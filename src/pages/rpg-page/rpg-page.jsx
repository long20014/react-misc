...
export default function RpgPage(props) { 
  ...
  const DoRefineArmor = (armor, armorPrototype, option) => {
    refineArmor(armor, armorPrototype, option);
    setNewState('armor', state.armor);
    console.log(state.armor); 
  }

  const getRandomItemFromList = (items) => {
    const luckyNumber = Math.floor(Math.random() * 10000) / 100;
    let threshold = 0;
    let itemIndex = 0;
    do {
  ...
  return (
    <div className="component-wrapper">      
      <h1>RpgPage</h1>
      <button onClick={() => openChest(state.chest, state.heroList[0])}>Open Treasure Chest</button>     
      ...
      <button onClick={() => DoRefineArmor(state.armor, armors[1], 'maxHp')}>Test Refine Armor</button>     
      <table>
        <thead>
          <tr>
            <th>item</th>
      ...
      <div className="skills-wrapper">
        {state.currentSkills.map(item => (
          <div className="skill-slot" key={item.name}>{item.name}</div>    
        ))}            
      </div>      
